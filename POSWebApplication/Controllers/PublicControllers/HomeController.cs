...
namespace POSWebApplication.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var dbUser = await _dbContext.pos_user.FirstOrDefaultAsync(user => user.UserCde == userCde);
            ...
            if (autoNumber != null)
            {
                autoNumber.BizDteString = ChangeDateFormat(bizDte);
            }

            // Main methods
            var billHList = await _dbContext.billh
                .Where(billH => billH.Status == 'P' && billH.BizDte.Date == bizDte.Date && billH.POSId == posId)
            ...
            var home = new Home()
            {
                ...
                BillHList = billHList,
                SalesTotal = GetTotal('P', bizDte, posId ?? ""),
                ReturnTotal = GetTotal('R', bizDte, posId ?? ""),
                VoidTotal = GetTotal('V', bizDte, posId ?? "")
            };
